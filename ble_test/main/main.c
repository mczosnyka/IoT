...
void trim(uint8_t *str) {
    int start = 0; // Indeks pierwszego znaku, który nie jest biały
    int end = strlen((char *)str) - 1; // Indeks ostatniego znaku ciągu

    // Znajdź pierwszy znak, który nie jest biały
    while (str[start] && isspace(str[start])) {
        start++;
    }

    // Znajdź ostatni znak, który nie jest biały
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (adv_name != NULL) {
                trim(adv_name);
                adv_name_len = strlen((const char*)adv_name);
                // ESP_LOGI(GATTC_TAG, "znaleziona: %s", adv_name);
                // ESP_LOGI(GATTC_TAG, "ustawiona: %s", remote_device_name);
                // ESP_LOGI(GATTC_TAG, "dlugosc znaleziona: %d", adv_name_len);
                // ESP_LOGI(GATTC_TAG, "dlugosc ustawiona: %d", strlen(remote_device_name));
                // ESP_LOGI(GATTC_TAG, "compare: %d", strncmp((char *)adv_name, remote_device_name, adv_name_len));
                for (int i = 0; i < strlen((char *)adv_name); i++) {
                    ESP_LOGW(GATTC_TAG, "0x%02X ", adv_name[i]);
                }
